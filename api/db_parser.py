...
sqlite_create0 = '''CREATE TABLE IF NOT EXISTS office_data (id INTEGER PRIMARY KEY, salePointName VARCHAR,
    address VARCHAR, latitude FLOAT, longitude FLOAT, distance INTEGER, rko VARCHAR, officeType VARCHAR,
    salePointFormat VARCHAR, suoAvailability BIT, hasRamp BIT, metroStation VARCHAR, kep VARCHAR);'''
...
def parse0():
    try:
        ...
        for data in config.offices:
            ...
            `longitude`, `distance`, `rko`, `officeType`, `salePointFormat`, `suoAvailability`,
            ...
            cursor.execute(query, (
                ...
                'Есть' if data['suoAvailability'] == 'Y' else ('Нет' if data['suoAvailability'] == 'N' else 'Неизвестно'),
